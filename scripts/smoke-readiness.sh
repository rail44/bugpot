...
echo "  http: $result"
status=$(echo "$result" | awk '{print $1}')
# `UpstreamResolver::resolve` distinguishes "no app for this host"
# (404) from "the app is registered but failed to come up" (502).
# A failed readiness probe lands in the latter — the spec is on
# disk, ensure_running returned Err, so the router surfaces it as
# `ResolveError::Unhealthy` → 502 Bad Gateway. Anything else here
# (notably 404) means the controller is collapsing both errors
# again, which is exactly the regression this test catches.
if [ "$status" != "502" ]; then
    echo "FAIL: expected 502 for bad app (registered but unhealthy → router 502), got $status"
    echo
    echo "=== tail of log ==="
    tail -40 "$LOG"
    exit 1
